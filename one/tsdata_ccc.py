...
def main(date=None):
    if date is not None:
        dfs = ts.get_today_all()
    else:
        dfs = ts.get_day_all(date)
    if len(dfs) == 0:
        return
    columns = ['trade_date', 'code', 'name', 'open', 'close', 'return', 'high', 'low', 'pre_close', 'vol', 'amount']
    df = dfs.rename(
        columns={'data': columns[0], 'code': columns[1], 'changepercent': columns[4], 'settlement': columns[5],
                 'volume': columns[9]})
    s_codes = []
    for i in df['code']:
        if len(str(i)) < 6:
...
if __name__ == '__main__':
    ...
    ts_data = ts.pro_api()
    df_ts = ts_data.trade_cal(exchange='', start_date='20210201', end_date=cur_d, is_open='1')
